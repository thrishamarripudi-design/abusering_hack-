...
def _merchants():
    return pd.DataFrame({
        "merchant_id": ["M1"], "category": ["grocery"], "risk_profile": ["low"],
...
def test_future_chargeback_does_not_leak_into_past_transaction():
    ...
    chargebacks = pd.DataFrame({
        "chargeback_id": ["CB1"], "transaction_id": ["T1"], "timestamp": [late_ts],
        "amount": [100.0], "reason": ["unauthorized"],
    })
...
def test_future_refund_does_not_leak_into_earlier_transaction():
    ...
    refunds = pd.DataFrame({
        "refund_id": ["R1"], "transaction_id": ["T2"], "timestamp": [late_ts],
        "amount": [120.0], "reason": ["not_as_described"],
    })
...
def test_future_device_relationship_does_not_affect_earlier_transaction():
    ...
    customers = pd.concat([_customers(), pd.DataFrame({
        "customer_id": ["C2"], "account_created_at": [pd.Timestamp("2024-06-01")],
        "customer_segment": ["normal"], "country": ["IN"],
    })], ignore_index=True)
    merchants = _merchants()
    refunds = pd.DataFrame(columns=["refund_id", "transaction_id", "timestamp", "amount", "reason"])
    chargebacks = pd.DataFrame(columns=["chargeback_id", "transaction_id", "timestamp", "amount", "reason"])
...
def test_feature_columns_present():
    txns = _tiny_transactions()
    feats = build_features(txns, _customers(), _merchants(),
