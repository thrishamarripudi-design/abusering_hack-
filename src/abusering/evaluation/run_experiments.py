...
def main():
    ...
    X_train, y_train, amt_train, _ = build_xy(features, ground_truth, splits["train_transaction_ids"], FEATURE_COLUMNS)
    X_val, y_val, amt_val, _ = build_xy(features, ground_truth, splits["validation_transaction_ids"], FEATURE_COLUMNS)
    X_test, y_test, amt_test, _ = build_xy(features, ground_truth, splits["test_transaction_ids"], FEATURE_COLUMNS)
    print(f"[experiments] train n={len(y_train)} pos={y_train.sum()} | "
          f"val n={len(y_val)} pos={y_val.sum()} | test n={len(y_test)} pos={y_test.sum()}")

    val_df = features[features.transaction_id.isin(set(splits["validation_transaction_ids"]))].reset_index(drop=True)
    val_df = val_df.merge(ground_truth[["transaction_id", "abuse_label"]], on="transaction_id")

    results = {}

    def log_experiment(name, purpose, y_score_val, feature_set_used, extra=None, model_bundle=None,
                        hyperparams=None):
        ...
        entry = {
            ...
            "threshold_curve": thr_result["curve"],  # full curve: precision/recall/F1/FP/FN/loss per threshold
            "pr_curve": pr_curve,
        ...
        print(f"[experiments] {name}: PR-AUC={val_metrics['pr_auc']:.4f} "
    ...
    log_experiment("baseline_rules", "Reasonable non-ML operational approach used for comparison.",
                    score, ["shared_device_count", "shared_ip_count", "ip_transaction_velocity_1h",
                            "shared_instrument_count", "transactions_in_time_window_15m"])

    # --- E1: Logistic Regression ---
    bundle = tr.train_logreg(X_train, y_train, seed=args.seed)
    score = tr.predict_logreg(bundle, X_val)
    log_experiment("E1_logistic_regression", "Interpretable linear baseline; coefficient sanity check.",
                    score, FEATURE_COLUMNS, model_bundle=bundle)

    # --- E2: Random Forest ---
    bundle = tr.train_random_forest(X_train, y_train, seed=args.seed)
    score = tr.predict_random_forest(bundle, X_val)
    log_experiment("E2_random_forest", "Nonlinear interactions, robust tree baseline.",
                    score, FEATURE_COLUMNS, model_bundle=bundle)
    ...
    log_experiment("E3_xgboost", "Strong structured/tabular model on engineered risk features.",
                    score, FEATURE_COLUMNS, model_bundle=bundle)

    # --- E4: CatBoost ---
    bundle = tr.train_catboost(X_train, y_train, seed=args.seed)
    score = tr.predict_catboost(bundle, X_val)
    log_experiment("E4_catboost", "Compare another strong GBDT approach / categorical handling.",
                    score, FEATURE_COLUMNS, model_bundle=bundle)
    ...
    log_experiment("E5_xgboost_no_graph", "Does relational/graph information actually help? (A/B vs E3)",
                    score, BEHAVIORAL_FEATURE_COLUMNS, model_bundle=bundle)

    graph_uplift_pr_auc = results["E3_xgboost"]["validation_metrics"]["pr_auc"] - \
    ...
    best_params, best_val_ap = run_optuna_xgb(X_train, y_train, X_val, y_val, n_trials=args.optuna_trials, seed=args.seed)
    bundle = tr.train_xgboost(X_train, y_train, seed=args.seed, params=best_params)
    score = tr.predict_xgboost(bundle, X_val)
    log_experiment("E3b_xgboost_optuna_tuned", "XGBoost with Optuna-tuned hyperparameters (all features).",
                    score, FEATURE_COLUMNS, model_bundle=bundle, hyperparams=best_params)
    ...
    for name, cols in ablation_defs.items():
        ...
        print(f"[ablation] {name}: n_feat={len(cols)} PR-AUC={m['pr_auc']:.4f} "
    ...
    winner_name = min(candidates, key=lambda k: candidates[k]["validation_metrics"]["expected_financial_loss"])
    ...
    X_all, y_all, amt_all, ids_all = build_xy(
        ...
        splits["train_transaction_ids"] + splits["validation_transaction_ids"] + splits["test_transaction_ids"],
        FEATURE_COLUMNS,
    )
    ...
    print(f"[experiments] wrote {artifacts_dir / 'scored_transactions.parquet'} "
